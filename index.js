...
client.on("message", message => {
  ...
  if (message.content === prefix + "help") {
    ...
    embed.addField(
      `${prefix}uptime + Link`,
      [
        `**\`!uptime your bot project link it keeps your bot 24 Hours running\`**`
      ],
      false
    );
    ...
    fetch(link)
      .then(() => {
        if (
          ...
          embed.setTitle(
            "Link is Already on the site :small_red_triangle_down:"
          );
          embed.setDescription(
            `[**This link is already on system !! 🔴**](${link})`
          );
          message.channel.send(embed);
          return;
        }
        ...
        embed.setDescription(
          `[**Done uptime and now your project online 24 hours. 🟢**](${link})`
        );
        message.channel.send(embed);
